const state = {
  ...
  materialStudio: {
    ...
    files: [],
    agentTrace: [],
    runOutput: "Noch keine Material-Ausfuehrung.",
    lastRun: null,
    busy: false,
...
const ui = {
  ...
  assistantModelUrl: $("assistant-model-url"),
  assistantModelFile: $("assistant-model-file"),
  assistantLoadUrlButton: $("assistant-load-url"),
  assistantLoadFileButton: $("assistant-load-file"),
  assistantUnloadButton: $("assistant-unload-model"),
  ...
  materialAdoptButton: $("adopt-material"),
  materialProfile: $("material-profile"),
  materialInstructionCard: $("material-instruction-card"),
  materialInstructionPreset: $("material-instruction-preset"),
  materialInstructionMeta: $("material-instruction-meta"),
  materialApplyInstruction: $("apply-material-instruction"),
  materialLanguage: $("material-language"),
  materialAttemptLimit: $("material-attempt-limit"),
  materialProfileMeta: $("material-profile-meta"),
  materialStatus: $("material-status"),
  materialTitle: $("material-title"),
  materialSummary: $("material-summary"),
  materialMainFile: $("material-main-file"),
  materialCode: $("material-code"),
  materialStdin: $("material-stdin"),
  materialTeacherNotes: $("material-teacher-notes"),
  materialStudentMaterial: $("material-student-material"),
  materialAgentTrace: $("material-agent-trace"),
  materialRunOutput: $("material-run-output"),
};
...
function materialStudioAgentLabelForPhase(phase = "") {
  const key = String(phase || "").toLowerCase();
  if (key.startsWith("plan")) return "Planer";
  if (key.startsWith("author")) return "Autor";
  if (key.startsWith("repair")) return "Debugger";
  if (key.startsWith("pedagogy")) return "Didaktik";
  return "Lokale KI";
}

function materialStudioActiveTraceItem(aiStatus) {
  const active = state.materialStudio.activeInference;
  if (!active) return null;
  const model = active.modelLabel || aiStatus.model_label || "";
  const elapsed = formatElapsedLabel(active.startedAt);
  return {
    agent: materialStudioAgentLabelForPhase(active.phase),
    stage: active.phase || "infer",
    status: "running",
    model: [model, elapsed ? `laeuft seit ${elapsed}` : ""].filter(Boolean).join(" | "),
    summary: active.status || "Lokaler KI-Server verarbeitet Material...",
  };
}
const MANAGED_SETTING_KEYS = new Set([
  ...
  "llamacpp_gpu_layers",
  "llamacpp_threads",
  "llamacpp_sleep_idle_seconds",
  "llamacpp_port",
  "llamacpp_server_path",
  "litertlm_model_path",
  "litertlm_model_alias",
  "litertlm_backend",
  "litertlm_sleep_idle_seconds",
  "litertlm_port",
  "litertlm_binary_path",
  "litertlm_home_path",
  "ondevice_max_tokens",
  "ondevice_top_k",
...
function materialStudioMainFileForLanguage(language) {
  return {
    python: "main.py",
    javascript: "main.js",
    cpp: "main.cpp",
...
function materialStudioProfiles() {
  return Array.isArray(state.bootstrap?.material_studio_profiles) ? state.bootstrap.material_studio_profiles : [];
}

function materialStudioInstructionPresets() {
  return Array.isArray(state.bootstrap?.material_studio_instruction_presets) ? state.bootstrap.material_studio_instruction_presets : [];
}

function availableMaterialInstructionPresets() {
  const profile = state.materialStudio.profile || "example-code";
  const language = state.materialStudio.language || "python";
  return materialStudioInstructionPresets().filter((item) => {
...
function currentMaterialInstructionPreset() {
  const presets = availableMaterialInstructionPresets();
  return presets.find((item) => item.key === state.materialStudio.instructionPresetKey) || null;
}

function applySelectedMaterialInstruction() {
  const preset = currentMaterialInstructionPreset();
  if (!preset) throw new Error("Fuer dieses Agentenprofil gibt es aktuell keine vorkonfigurierte Kursanweisung.");
  const current = ui.materialPrompt?.value || state.materialStudio.prompt || "";
  const nextPrompt = current.trim() ? `${current.trim()}\n\n${preset.prompt}` : preset.prompt;
  if (current.includes(preset.prompt)) {
    notify(`Die Kursanweisung '${preset.label}' ist bereits im Lehrer-Prompt enthalten.`);
    return;
  }
  state.materialStudio.prompt = nextPrompt;
  if (ui.materialPrompt) ui.materialPrompt.value = nextPrompt;
  notify(`Kursanweisung eingefuegt: ${preset.label}`);
}

function currentMaterialProfile() {
  const profiles = materialStudioProfiles();
  return profiles.find((item) => item.key === state.materialStudio.profile) || profiles[0] || null;
}
...
function populateServerSettingsPanel(settings = {}, runtime = {}) {
  ...
  $("setting-ai-provider").value = settings.ai_provider || "auto";
  $("setting-litertlm-model-path").value = settings.litertlm_model_path || "";
  $("setting-litertlm-model-alias").value = settings.litertlm_model_alias || "";
  $("setting-litertlm-backend").value = settings.litertlm_backend || "cpu";
  $("setting-litertlm-sleep-idle").value = settings.litertlm_sleep_idle_seconds || 45;
  $("setting-litertlm-port").value = settings.litertlm_port || 9379;
  $("setting-litertlm-binary-path").value = settings.litertlm_binary_path || "";
  $("setting-litertlm-home-path").value = settings.litertlm_home_path || "";
  $("setting-llamacpp-model-path").value = settings.llamacpp_model_path || "";
  $("setting-llamacpp-model-alias").value = settings.llamacpp_model_alias || "";
  $("setting-llamacpp-backend").value = settings.llamacpp_backend || "vulkan";
  $("setting-llamacpp-ctx-size").value = settings.llamacpp_ctx_size || 4096;
  $("setting-llamacpp-gpu-layers").value = settings.llamacpp_gpu_layers ?? 99;
  $("setting-llamacpp-threads").value = settings.llamacpp_threads ?? 0;
  $("setting-llamacpp-sleep-idle").value = settings.llamacpp_sleep_idle_seconds || 45;
  $("setting-llamacpp-port").value = settings.llamacpp_port || 11435;
  $("setting-llamacpp-server-path").value = settings.llamacpp_server_path || "";
  $("setting-ondevice-max-tokens").value = settings.ondevice_max_tokens || 1024;
...
function arrangeRightPanel(session) {
  ...
  if (session?.permissions?.["ai.use"]) orderedSections.push(ui.assistantPanel);
...
function materialStudioFilesWithCurrentMainFile() {
  const language = ui.materialLanguage?.value || state.materialStudio.language || "python";
  const mainFile = (ui.materialMainFile?.value || state.materialStudio.mainFile || materialStudioMainFileForLanguage(language)).trim() || materialStudioMainFileForLanguage(language);
  const code = ui.materialCode?.value ?? state.materialStudio.code ?? "";
  const existing = Array.isArray(state.materialStudio.files) ? state.materialStudio.files : [];
  const files = [];
  let mainWritten = false;
  existing.forEach((item) => {
    if (!item?.path) return;
    if (item.path === mainFile) {
      files.push({ path: mainFile, content: code });
      mainWritten = true;
      return;
    }
  ...
  if (!mainWritten) {
    files.unshift({ path: mainFile, content: code });
  }
  return files;
}

function applyMaterialStudioPayload(payload = {}) {
  state.materialStudio.title = payload.title || "Unterrichtsmaterial";
  state.materialStudio.summary = payload.summary || "";
  state.materialStudio.profile = payload.profile?.key || state.materialStudio.profile || "example-code";
  state.materialStudio.instructionPresetKey = payload.instruction_preset_key || state.materialStudio.instructionPresetKey || "";
  state.materialStudio.language = payload.language || state.materialStudio.language || "python";
  state.materialStudio.mainFile = payload.main_file || materialStudioMainFileForLanguage(state.materialStudio.language);
  state.materialStudio.code = payload.code || "";
  state.materialStudio.stdin = payload.stdin || "";
  state.materialStudio.teacherNotes = payload.teacher_notes_markdown || "";
  state.materialStudio.studentMaterial = payload.student_material_markdown || "";
  state.materialStudio.files = Array.isArray(payload.files) ? payload.files.map((item) => ({ path: item.path, content: item.content || "" })) : materialStudioFilesWithCurrentMainFile();
  state.materialStudio.agentTrace = Array.isArray(payload.agent_trace) ? payload.agent_trace : [];
  state.materialStudio.lastRun = payload.run || null;
  state.materialStudio.activeInference = null;
  if (payload.run) {
    const outputParts = [];
    if (payload.run.stdout) outputParts.push(payload.run.stdout);
    if (payload.run.stderr) outputParts.push(payload.run.stderr);
    const notes = Array.isArray(payload.run.notes) ? payload.run.notes : [];
    if (notes.length) {
      outputParts.push(`Hinweise:\n- ${notes.join("\n- ")}`);
    }
    state.materialStudio.runOutput = outputParts.join("\n\n").trim() || "Material-Lauf ohne sichtbare Ausgabe abgeschlossen.";
  }
}

function renderMaterialStudio() {
  const allowed = canUseMaterialStudio();
  ui.materialStudioPanel?.classList.toggle("hidden", !allowed);
  $("open-material-studio")?.classList.toggle("hidden", !allowed);
  if (!allowed) return;
  const aiStatus = currentAiConfig() || {};
  const aiBusy = false;
  if (state.materialStudio.statusTimer) {
    clearTimeout(state.materialStudio.statusTimer);
    state.materialStudio.statusTimer = null;
  }
  if (state.materialStudio.busy && state.materialStudio.activeInference) {
    state.materialStudio.statusTimer = setTimeout(() => {
      state.materialStudio.statusTimer = null;
      if (state.materialStudio.busy) renderMaterialStudio();
    }, 1000);
  }
  if (ui.materialGenerateButton) ui.materialGenerateButton.disabled = state.materialStudio.busy || aiBusy;
  if (ui.materialRunButton) ui.materialRunButton.disabled = state.materialStudio.busy || aiBusy;
  if (ui.materialAdoptButton) ui.materialAdoptButton.disabled = state.materialStudio.busy;

  if (ui.materialPrompt && document.activeElement !== ui.materialPrompt) ui.materialPrompt.value = state.materialStudio.prompt || "";
  const profiles = materialStudioProfiles();
  if (ui.materialProfile) {
    ui.materialProfile.innerHTML = profiles.map((item) => `<option value="${escapeHtml(item.key)}">${escapeHtml(item.label)}</option>`).join("");
    if (!profiles.some((item) => item.key === state.materialStudio.profile)) {
      state.materialStudio.profile = profiles[0]?.key || "example-code";
    }
    ui.materialProfile.value = state.materialStudio.profile || "example-code";
  }
  if (ui.materialLanguage) ui.materialLanguage.value = state.materialStudio.language || "python";
  const instructionPresets = availableMaterialInstructionPresets();
  if (ui.materialInstructionCard) ui.materialInstructionCard.classList.toggle("hidden", !instructionPresets.length);
  if (!instructionPresets.some((item) => item.key === state.materialStudio.instructionPresetKey)) {
    state.materialStudio.instructionPresetKey = "";
  }
  if (ui.materialInstructionPreset) {
    ui.materialInstructionPreset.innerHTML = instructionPresets.length
      ? [`<option value="">Kursanweisung auswaehlen</option>`, ...instructionPresets.map((item) => `<option value="${escapeHtml(item.key)}">${escapeHtml(item.label)}</option>`)].join("")
      : `<option value="">Keine Anweisung verfuegbar</option>`;
    ui.materialInstructionPreset.value = state.materialStudio.instructionPresetKey || "";
    ui.materialInstructionPreset.disabled = !instructionPresets.length;
  }
  const instructionPreset = currentMaterialInstructionPreset();
  if (ui.materialInstructionMeta) {
    ui.materialInstructionMeta.textContent = instructionPreset
      ? `${instructionPreset.label}: ${instructionPreset.summary} Diese Vorgabe wird serverseitig verbindlich angewendet.`
      : (instructionPresets.length
        ? "Waehle eine Kursanweisung aus. Die gewaehlte Vorgabe wird serverseitig verbindlich angewendet."
        : "Fuer diese Kombination aus Agentenprofil und Sprache ist keine Kursanweisung hinterlegt.");
  }
  if (ui.materialApplyInstruction) ui.materialApplyInstruction.disabled = !instructionPreset;
  if (ui.materialAttemptLimit && document.activeElement !== ui.materialAttemptLimit) ui.materialAttemptLimit.value = state.materialStudio.attemptLimit || 3;
  if (ui.materialMainFile && document.activeElement !== ui.materialMainFile) ui.materialMainFile.value = state.materialStudio.mainFile || materialStudioMainFileForLanguage(state.materialStudio.language || "python");
  if (ui.materialCode && document.activeElement !== ui.materialCode) ui.materialCode.value = state.materialStudio.code || "";
  if (ui.materialStdin && document.activeElement !== ui.materialStdin) ui.materialStdin.value = state.materialStudio.stdin || "";
  const profile = currentMaterialProfile();
  ui.materialProfileMeta.textContent = profile ? `${profile.label}: ${profile.description}` : "Noch kein Profil gewaehlt.";
  if (state.materialStudio.activeInference) {
    const elapsed = formatElapsedLabel(state.materialStudio.activeInference.startedAt);
    const model = state.materialStudio.activeInference.modelLabel || aiStatus.model_label || "";
    ui.materialStatus.textContent = [
      state.materialStudio.activeInference.status || state.materialStudio.status || "Lokaler KI-Server verarbeitet Material...",
      model ? `Modell: ${model}` : "",
      elapsed ? `aktiv seit ${elapsed}` : "",
    ].filter(Boolean).join(" | ");
  } else {
    ui.materialStatus.textContent = state.materialStudio.status || "Noch kein Material erzeugt.";
  }
  ui.materialTitle.textContent = state.materialStudio.title || "Noch kein Artefakt";
  ui.materialSummary.textContent = state.materialStudio.summary || "Das Studio erzeugt Unterrichtsmaterial und direkt ausfuehrbaren Beispielcode.";
  ui.materialTeacherNotes.innerHTML = state.materialStudio.teacherNotes ? markdownToHtml(state.materialStudio.teacherNotes) : "<p class=\"muted\">Noch keine Lehrkraft-Hinweise.</p>";
  ui.materialStudentMaterial.innerHTML = state.materialStudio.studentMaterial ? markdownToHtml(state.materialStudio.studentMaterial) : "<p class=\"muted\">Noch kein Schueler-Material.</p>";
  const traceItems = [];
  const activeTrace = materialStudioActiveTraceItem(aiStatus);
  if (activeTrace) traceItems.push(activeTrace);
  traceItems.push(...(state.materialStudio.agentTrace || []));
  ui.materialAgentTrace.innerHTML = traceItems.map((item) => `
    <article class="material-agent-card material-agent-${escapeHtml(item.status || "completed")}">
      <div class="material-agent-head">
        <strong>${escapeHtml(item.agent || "Agent")}</strong>
        <span>${escapeHtml(item.stage || "")}</span>
      </div>
      <p>${escapeHtml(item.summary || "Ohne Zusammenfassung.")}</p>
      <small>${escapeHtml(item.model || (item.attempt ? `Versuch ${item.attempt}` : ""))}</small>
    </article>
  `).join("") || "<p class=\"muted\">Noch kein Agentenlauf.</p>";
  ui.materialRunOutput.textContent = state.materialStudio.runOutput || "Noch keine Material-Ausfuehrung.";
}

async function completeSingleInference(preparedPayload, endpoint, finalBody) {
  if (!preparedPayload || preparedPayload.mode !== "infer") return preparedPayload;
  let payload = preparedPayload;
  while (payload && payload.mode === "infer") {
    state.materialStudio.activeInference = {
      phase: payload.phase || "infer",
      status: payload.status || "Lokaler KI-Server verarbeitet Material...",
      modelLabel: currentAiConfig()?.model_label || "",
      startedAt: Date.now(),
    };
    state.materialStudio.status = payload.status || state.materialStudio.status;
    if (Array.isArray(payload.agent_trace)) {
      state.materialStudio.agentTrace = payload.agent_trace;
    }
    ...
    payload = await api(endpoint, {
      ...
      body: {
        ...
        generation_options: payload.generation_options || null,
...
async function resolveMaterialStudioInference(payload) {
  payload = await completeSingleInference(payload, "/api/material-studio/infer");
  state.materialStudio.activeInference = null;
  return payload;
}

async function generateMaterialStudio() {
  if (!canUseMaterialStudio()) throw new Error("Material-Studio ist fuer diese Sitzung nicht freigegeben.");
  if (state.materialStudio.busy) throw new Error("Material-Studio laeuft bereits. Bitte warte auf den aktuellen Vorgang.");
  const prompt = ui.materialPrompt.value.trim();
  if (!prompt) throw new Error("Bitte zuerst einen Lehrer-Prompt eingeben.");
  state.materialStudio.busy = true;
  try {
    state.materialStudio.prompt = prompt;
    state.materialStudio.profile = ui.materialProfile.value || state.materialStudio.profile || "example-code";
    state.materialStudio.language = ui.materialLanguage.value || "python";
    state.materialStudio.attemptLimit = Number(ui.materialAttemptLimit.value || 3);
    state.materialStudio.status = "Agenten planen Material und Code...";
    renderMaterialStudio();

    const payload = await api("/api/material-studio/generate", {
      method: "POST",
      body: {
        prompt,
        profile: state.materialStudio.profile,
        instruction_preset_key: state.materialStudio.instructionPresetKey,
        language: state.materialStudio.language,
        attempt_limit: state.materialStudio.attemptLimit,
        seed_code: ui.materialCode.value,
        seed_path: ui.materialMainFile.value,
      },
    });
    const finalPayload = await resolveMaterialStudioInference(payload);
    applyMaterialStudioPayload(finalPayload);
    state.materialStudio.status = finalPayload.passed
      ? `Material erfolgreich erzeugt und ausgefuehrt. Versuche: ${finalPayload.attempts_used}/${finalPayload.attempt_limit}.`
      : `Material erzeugt, aber der Code ist nach ${finalPayload.attempts_used} Versuch(en) noch nicht fehlerfrei.`;
    renderMaterialStudio();
    notify(state.materialStudio.status);
  } finally {
    state.materialStudio.busy = false;
    state.materialStudio.activeInference = null;
    renderMaterialStudio();
  }
}

async function runMaterialStudioCode() {
  if (!canUseMaterialStudio()) throw new Error("Material-Studio ist fuer diese Sitzung nicht freigegeben.");
  if (state.materialStudio.busy) throw new Error("Material-Studio verarbeitet bereits einen Vorgang. Bitte warte kurz.");
  const language = ui.materialLanguage.value || state.materialStudio.language || "python";
  const profile = ui.materialProfile.value || state.materialStudio.profile || "example-code";
  const mainFile = (ui.materialMainFile.value || materialStudioMainFileForLanguage(language)).trim() || materialStudioMainFileForLanguage(language);
  const code = ui.materialCode.value;
  if (!code.trim()) throw new Error("Kein Code im Material-Studio vorhanden.");
  state.materialStudio.busy = true;
  try {
    state.materialStudio.language = language;
    state.materialStudio.profile = profile;
    state.materialStudio.mainFile = mainFile;
    state.materialStudio.code = code;
    state.materialStudio.stdin = ui.materialStdin.value || "";
    state.materialStudio.files = materialStudioFilesWithCurrentMainFile();
    state.materialStudio.status = "Material-Studio fuehrt den aktuellen Code aus...";
    renderMaterialStudio();

    const payload = await api("/api/material-studio/run", {
      method: "POST",
      body: {
        title: ui.materialTitle.textContent || state.materialStudio.title || "Material-Studio Lauf",
        summary: state.materialStudio.summary || "",
        profile,
        language,
        main_file: mainFile,
        code,
        stdin: state.materialStudio.stdin,
        files: state.materialStudio.files,
        teacher_notes_markdown: state.materialStudio.teacherNotes || "",
        student_material_markdown: state.materialStudio.studentMaterial || "",
      },
    });
    applyMaterialStudioPayload(payload);
    state.materialStudio.status = payload.passed
      ? "Material-Code erfolgreich ausgefuehrt."
      : "Material-Code ausgefuehrt, aber mit Fehlern beendet.";
    renderMaterialStudio();
    notify(state.materialStudio.status);
  } finally {
    state.materialStudio.busy = false;
...
function adoptMaterialStudioCode() {
  if (!state.project) throw new Error("Bitte zuerst ein Projekt waehlen, bevor der Code in den Haupteditor uebernommen wird.");
  const mainFile = (ui.materialMainFile.value || state.materialStudio.mainFile || "").trim();
  const code = ui.materialCode.value;
  if (!mainFile || !code.trim()) throw new Error("Im Material-Studio liegt noch kein uebernehmbarer Code vor.");
  ui.filePath.value = mainFile;
  ui.fileEditor.value = code;
  state.filePath = mainFile;
  state.fileDirty = true;
  setRunOutput("Code aus dem Material-Studio in den Haupteditor uebernommen. Bitte speichern oder direkt ausfuehren.");
  notify(state.materialStudio.files.length > 1
    ? "Hauptdatei uebernommen. Zusatzdateien verbleiben im Material-Studio-Bundle."
    : "Code in den Haupteditor uebernommen.");
}
...
function renderAssistantStatus() {
  ...
  const status = currentAiConfig() || {};
  const providerLabel = status.provider === "server-litert-lm" ? "LiteRT-LM" : "llama.cpp";
  const modelLabel = status.model_label || status.model_path || "lokales Modell";
  if (!aiAllowed) {
    ui.assistantStatus.textContent = "Lokale KI ist fuer diese Sitzung nicht freigegeben.";
    return;
  }
  const busy = Boolean(state.assistantBusy || state.materialStudio.busy);
  if (ui.assistantLoadUrlButton) ui.assistantLoadUrlButton.disabled = true;
  if (ui.assistantLoadFileButton) ui.assistantLoadFileButton.disabled = true;
  if (ui.assistantUnloadButton) ui.assistantUnloadButton.disabled = true;
  ui.assistantForm?.querySelector("button[type=submit]")?.toggleAttribute("disabled", busy);
  if (state.assistantBusy) {
    ui.assistantStatus.textContent = "Lokaler KI-Server verarbeitet gerade eine Anfrage...";
    return;
  }
  if (status.running) {
    ui.assistantStatus.textContent = `Lokaler KI-Server aktiv. Provider: ${providerLabel}. Modell: ${modelLabel}.`;
    return;
  }
  if (status.configured) {
    ui.assistantStatus.textContent = `Lokaler KI-Server bereit. Provider: ${providerLabel}. Standardmodell: ${modelLabel}. Backend: ${status.backend || "auto"}.`;
    return;
  }
...
async function askAssistant(event) {
  event.preventDefault();
  if (state.assistantBusy) throw new Error("Lokaler KI-Server verarbeitet bereits eine Anfrage. Bitte warte kurz.");
  ...
  try {
    if (mode === "mentor") {
      ...
      const prepared = await api(`/api/projects/${state.project.project_id}/mentor/ask`, {
        method: "POST",
        body: {
          prompt,
          code: ui.fileEditor.value,
    ...
    ui.assistantOutput.textContent = `Lokale KI Fehler:\n${error.message}`;
    notify(error.message);
  } finally {
...
$("generate-material")?.addEventListener("click", () => generateMaterialStudio().catch((error) => {
  state.materialStudio.status = `Material-Studio Fehler: ${error.message}`;
  state.materialStudio.runOutput = `Material-Studio Fehler:\n${error.message}`;
  renderMaterialStudio();
  notify(error.message);
}));
$("run-material")?.addEventListener("click", () => runMaterialStudioCode().catch((error) => {
  state.materialStudio.status = `Material-Studio Fehler: ${error.message}`;
  state.materialStudio.runOutput = `Material-Studio Fehler:\n${error.message}`;
  renderMaterialStudio();
  notify(error.message);
}));
$("adopt-material")?.addEventListener("click", () => {
  try {
    adoptMaterialStudioCode();
  } catch (error) {
    notify(error.message);
  }
});
...
ui.materialPrompt?.addEventListener("input", () => { state.materialStudio.prompt = ui.materialPrompt.value; });
ui.materialProfile?.addEventListener("change", () => {
  state.materialStudio.profile = ui.materialProfile.value || "example-code";
  state.materialStudio.instructionPresetKey = "";
  renderMaterialStudio();
});
ui.materialLanguage?.addEventListener("change", () => {
  state.materialStudio.language = ui.materialLanguage.value || "python";
  state.materialStudio.instructionPresetKey = "";
  const fallback = materialStudioMainFileForLanguage(state.materialStudio.language);
  if (!ui.materialMainFile.value || ui.materialMainFile.value === state.materialStudio.mainFile) {
    state.materialStudio.mainFile = fallback;
    ui.materialMainFile.value = fallback;
  }
  renderMaterialStudio();
});
ui.materialInstructionPreset?.addEventListener("change", () => {
  state.materialStudio.instructionPresetKey = ui.materialInstructionPreset.value || "";
  renderMaterialStudio();
});
ui.materialApplyInstruction?.addEventListener("click", () => {
  try {
    applySelectedMaterialInstruction();
  } catch (error) {
    notify(error.message);
  }
});
ui.materialAttemptLimit?.addEventListener("input", () => {
  state.materialStudio.attemptLimit = Number(ui.materialAttemptLimit.value || 3);
});
ui.materialMainFile?.addEventListener("input", () => {
  state.materialStudio.mainFile = ui.materialMainFile.value;
});
ui.materialCode?.addEventListener("input", () => {
  state.materialStudio.code = ui.materialCode.value;
  state.materialStudio.files = materialStudioFilesWithCurrentMainFile();
});
ui.materialStdin?.addEventListener("input", () => {
  state.materialStudio.stdin = ui.materialStdin.value;
});
...
$("assistant-load-file")?.addEventListener("click", async () => {
  const provider = currentAiConfig()?.provider;
  if (provider === "server-litert-lm") {
    notify("Bitte die .litertlm-Datei und die lit-Binary konfigurieren oder in den LiteRT-Standardpfad legen.");
    return;
  }
  notify("Bitte das Modell im Ordner Model ablegen oder den Modellpfad in den Servereinstellungen setzen.");
});
$("assistant-unload-model")?.addEventListener("click", () => {
  notify("Der lokale KI-Prozess entlaedt das Modell nach der eingestellten Leerlaufzeit automatisch. Die naechste Anfrage startet es wieder.");
});
...
$("settings-form").addEventListener("submit", async (event) => {
  ...
  const payload = await api("/api/server/settings", {
    ...
    body: {
      ...
      litertlm_model_path: $("setting-litertlm-model-path").value,
      litertlm_model_alias: $("setting-litertlm-model-alias").value,
      litertlm_backend: $("setting-litertlm-backend").value,
      litertlm_sleep_idle_seconds: Number($("setting-litertlm-sleep-idle").value || 45),
      litertlm_port: Number($("setting-litertlm-port").value || 9379),
      litertlm_binary_path: $("setting-litertlm-binary-path").value,
      litertlm_home_path: $("setting-litertlm-home-path").value,
      llamacpp_model_path: $("setting-llamacpp-model-path").value,
      llamacpp_model_alias: $("setting-llamacpp-model-alias").value,
      llamacpp_backend: $("setting-llamacpp-backend").value,
      llamacpp_ctx_size: Number($("setting-llamacpp-ctx-size").value || 4096),
      llamacpp_gpu_layers: Number($("setting-llamacpp-gpu-layers").value || 99),
      llamacpp_threads: Number($("setting-llamacpp-threads").value || 0),
      llamacpp_sleep_idle_seconds: Number($("setting-llamacpp-sleep-idle").value || 45),
      llamacpp_port: Number($("setting-llamacpp-port").value || 11435),
      llamacpp_server_path: $("setting-llamacpp-server-path").value,
      ondevice_max_tokens: Number($("setting-ondevice-max-tokens").value || 1024),
