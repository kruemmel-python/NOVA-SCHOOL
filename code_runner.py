...
class CodeRunner:
    ...
    def run_bundle(self, session: Any, payload: dict[str, Any]) -> dict[str, Any]:
        run_id = uuid.uuid4().hex[:10]
        language = self._resolve_bundle_language(payload)
        backend = self._runner_backend(payload) if language == "html" else self.resolve_backend(session, payload, purpose="Material-Studio")
        permission_key = LANGUAGE_TO_PERMISSION.get(language)
        if permission_key is None or not session.permissions.get(permission_key, False):
            raise PermissionError(f"Ausfuehrung fuer {language} ist nicht freigegeben.")
        ...
        tool_session = self.tool_sandbox.authorize(
            ...
            allowed_tools=allowed_tool_names(session.permissions),
            requested_tools={permission_key},
            metadata={"language": language, "backend": backend, "source": "material-studio"},
        )

        user_root = re.sub(r"[^a-z0-9]+", "-", str(getattr(session, "username", "teacher") or "teacher").strip().lower()).strip("-") or "teacher"
        run_root = self.config.data_path / "material_studio_runs" / user_root / run_id
        source_root, source_path = self._prepare_bundle_workspace(payload, language, run_root)
        ...
        if language == "html":
            result = RunResult(
                ...
                language=language,
                command=["preview"],
                stdout="HTML-Datei fuer das Material-Studio vorbereitet. Fuer eine Browser-Vorschau bitte den Code in ein Projekt uebernehmen.\n",
                preview_path="",
                notes=self._backend_notes(session.permissions, backend),
                tool_session=tool_session,
            )
        ...
        try:
            if backend == "container":
                result = self._run_containerized(run_id, language, source_path, run_root, source_root, stdin_text, env, tool_session, session.permissions, payload)
            elif language == "python":
                result = self._run_python(run_id, source_path, source_root, stdin_text, env, tool_session, session.permissions)
            elif language in {"javascript", "node"}:
                result = self._run_node_like(run_id, language, source_path, source_root, stdin_text, env, tool_session, session.permissions)
            elif language == "cpp":
                result = self._run_cpp(run_id, source_path, run_root, source_root, stdin_text, env, tool_session, session.permissions)
            elif language == "java":
                result = self._run_java(run_id, source_path, run_root, source_root, stdin_text, env, tool_session, session.permissions)
            elif language == "rust":
                result = self._run_rust(run_id, source_path, run_root, source_root, stdin_text, env, tool_session, session.permissions)
            elif language == "npm":
                result = self._run_npm(run_id, source_root, payload, stdin_text, env, tool_session, session.permissions)
            else:
                raise ValueError(f"unsupported language: {language}")
        finally:
            self.scheduler.release(lease)
    ...
    def _resolve_bundle_language(self, payload: dict[str, Any]) -> str:
        explicit = str(payload.get("language") or "").strip().lower()
        if explicit:
            return explicit
        path_text = str(payload.get("main_file") or payload.get("path") or "")
        return EXTENSION_TO_LANGUAGE.get(Path(path_text).suffix.lower(), "python")
    ...
    def _prepare_bundle_workspace(self, payload: dict[str, Any], language: str, run_root: Path) -> tuple[Path, Path]:
        workspace_root = run_root / "workspace"
        workspace_root.mkdir(parents=True, exist_ok=True)
        files = payload.get("files")
        if not isinstance(files, list) or not files:
            main_file = self._safe_relative_path(str(payload.get("main_file") or self._default_filename(language)))
            code = str(payload.get("code") or "")
            files = [{"path": main_file.as_posix(), "content": code}]
        for item in files:
            if not isinstance(item, dict):
                continue
            target = workspace_root / self._safe_relative_path(str(item.get("path") or self._default_filename(language)))
        ...
        main_file = self._safe_relative_path(str(payload.get("main_file") or self._default_filename(language)))
        source_path = workspace_root / main_file
        if not source_path.exists():
            raise FileNotFoundError(f"Hauptdatei fuer das Material-Studio nicht gefunden: {main_file.as_posix()}")
    ...
    def _run_containerized(
        ...
        image_notes, image_error = self._ensure_container_image_available(runtime_executable, image, project_root)
        if image_error is not None:
            return RunResult(
                run_id,
                language,
                image_error.command or [runtime_executable, "image", "inspect", image],
                image_error.stdout,
                image_error.stderr,
                image_error.returncode,
                image_error.duration_ms,
                notes=self._backend_notes(permissions, "container", runtime, image),
                tool_session=tool_session,
            )
        extra_notes.extend(image_notes)
        ...
        if language in {"javascript", "node"}:
            ...
            result = self._execute_container(run_id, language, runtime_executable, image, inner_command, project_root, container_workspace, stdin_text, env, tool_session, permissions)
            result.notes = list(result.notes or []) + extra_notes
            return result
        if language == "cpp":
        ...
        if language == "rust":
            ...
            if cargo_manifest.exists():
                ...
                result = self._execute_container(run_id, language, runtime_executable, image, ["cargo", "run", "--manifest-path", manifest_path], project_root, container_workspace, stdin_text, env, tool_session, permissions)
                result.notes = list(result.notes or []) + extra_notes
                return result
            container_source = self._container_path(project_root, source_path)
            build_root = project_root / ".nova-build"
        ...
        if language == "npm":
            ...
            result = self._execute_container(run_id, language, runtime_executable, image, ["npm", *tokens], project_root, container_workspace, stdin_text, env, tool_session, permissions)
            result.notes = list(result.notes or []) + extra_notes
            return result
    ...
    @staticmethod
    def _container_error_is_missing_image(raw_error: str) -> bool:
        lower = str(raw_error or "").strip().lower()
        return "no such image" in lower or "unable to find image" in lower

    def _ensure_container_image_available(self, runtime_executable: str, image: str, cwd: Path) -> tuple[list[str], "_RawResult | None"]:
        inspect_command = [runtime_executable, "image", "inspect", image]
        inspect_result = self._execute_raw(inspect_command, cwd, "", dict(os.environ), timeout_seconds=max(self.config.run_timeout_seconds, 60))
        inspect_result.command = inspect_command
        if inspect_result.returncode == 0:
            return [], None

        inspect_error = inspect_result.stderr or inspect_result.stdout or ""
        if not self._container_error_is_missing_image(inspect_error):
            inspect_result.stderr = self._container_runtime_error_message(runtime_executable, image, inspect_error)
            return [], inspect_result

        pull_command = [runtime_executable, "pull", image]
        pull_result = self._execute_raw(pull_command, self.config.base_path, "", dict(os.environ), timeout_seconds=max(self.config.run_timeout_seconds, 900))
        pull_result.command = pull_command
        if pull_result.returncode != 0:
            pull_error = pull_result.stderr or pull_result.stdout or inspect_error
            pull_result.stderr = self._container_runtime_error_message(runtime_executable, image, pull_error)
            return [], pull_result

        return [f"Container-Image wurde serverseitig automatisch nachgeladen: {image}."], None
    ...
    def _prepare_live_containerized(
        ...
        if image_error is not None:
            return LivePreparedRun(
                ...
                notes,
                tool_session,
                prelude_stdout=image_error.stdout,
                prelude_stderr=image_error.stderr,
                failed_returncode=image_error.returncode,
            )
    ...
    def _container_runtime_health_timeout_seconds(self) -> int:
        return max(30, min(int(self.config.run_timeout_seconds or 20), 90))
    ...
    def _container_runtime_health(self, runtime_executable: str, image: str) -> tuple[bool, str]:
        ...
        result = self._execute_raw(
            ...
            "",
            dict(os.environ),
            timeout_seconds=self._container_runtime_health_timeout_seconds(),
        )
        if result.returncode == 0:
            payload = (now + 15.0, True, "")
