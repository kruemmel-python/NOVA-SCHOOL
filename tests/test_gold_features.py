...
class GoldFeatureTests(unittest.TestCase):
    ...
    def test_mentor_persists_thread_and_teacher_can_inspect(self) -> None:
        ...
        service = SocraticMentorService(self.repository)
        session = _Session("student", display_name="Schueler")
        ...
        reply = service.store_reply(
            session,
            project,
            prompt="Warum scheitert meine Schleife?",
            reply="Pruefe zuerst die Schleifenbedingung und die Array-Grenzen.",
            model="fake-mentor",
        )

        self.assertIn("Schleifenbedingung", reply["reply"])
        self.assertIn("IndexError: list index out of range", prepared["prompt"])
        self.assertIn("for i in range", prepared["prompt"])
