...
class NovaSchoolApplication:
    ...
    def public_settings(self, session: SessionContext) -> dict[str, Any]:
        settings = {
            ...
            "ai_provider": self.repository.get_setting("ai_provider", self.default_ai_provider()),
            "llamacpp_model_alias": self.repository.get_setting("llamacpp_model_alias", "") or self.default_llamacpp_model_label(),
            "litertlm_model_alias": self.repository.get_setting("litertlm_model_alias", "") or self.default_litertlm_model_label(),
        }
        if session.is_teacher or session.permissions.get("ai.use", False):
            settings["llamacpp_model_path"] = self.repository.get_setting("llamacpp_model_path", "") or self.default_llamacpp_model_path()
            settings["litertlm_model_path"] = self.repository.get_setting("litertlm_model_path", "") or self.default_litertlm_model_path()
            settings["litertlm_binary_path"] = self.repository.get_setting("litertlm_binary_path", "") or self.default_litertlm_binary_path()
        return settings
    ...
    def bootstrap_payload(self, session: SessionContext) -> dict[str, Any]:
        ...
        if session.permissions.get("ai.use", False) or session.is_teacher:
            ai_status = self.ai.status(enabled=bool(session.permissions.get("ai.use", False)))
        return {
    ...
    def local_model_files(self) -> list[Path]:
        def sort_key(path: Path) -> tuple[int, str]:
            ...
            if path.suffix.lower() == ".gguf":
                ...
                score = 20
            elif path.suffix.lower() == ".litertlm" and not re.search(r"(?:^|[_-])(mt\d+|sm\d+)(?:[_.-]|$)", name):
                score = 30
        ...
        for root in [
            ...
            for path in resolved_root.iterdir():
                if not path.is_file() or path.suffix.lower() not in {".gguf", ".task", ".litertlm"}:
                    continue
    ...
    def default_llamacpp_model_path(self) -> str:
    ...
    def default_llamacpp_model_label(self) -> str:
        path = self.default_llamacpp_model_path()
        return Path(path).stem if path else ""

    def default_litertlm_model_path(self) -> str:
        files = self.local_model_files()
        if not files:
            return ""
        for item in files:
            if item.suffix.lower() == ".litertlm":
                return str(item)
        return ""

    def default_litertlm_model_label(self) -> str:
    ...
    def default_ai_provider(self) -> str:
        if self.default_litertlm_model_path() and self.default_litertlm_binary_path():
            return "litert-lm"
        if self.default_llamacpp_model_path():
            return "llama.cpp"
        return "auto"
...
class NovaSchoolRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_api(self, method: str, path: str, parsed: Any) -> None:
        ...
        if len(segments) >= 3 and segments[:2] == ["api", "projects"]:
            ...
            if len(segments) == 5 and segments[3] == "mentor" and segments[4] == "ask" and method == "POST":
                ...
                prepared = self.application.mentor.prepare(
                    session,
                    project,
                    prompt=prompt,
                ...
                reply_text, model_name = self.application.ai.complete(
                    ...
                    generation_options={"max_tokens": min(self.application.ai.max_tokens, 896)},
                    timeout_seconds=120.0,
                )
                self._send_json(
                    HTTPStatus.OK,
                    self.application.mentor.store_reply(
                        session,
                        project,
                        prompt=prompt,
                        reply=reply_text,
        ...
        if method == "GET" and path == "/api/assistant/status":
            self._require_permission(session, "ai.use")
            self._send_json(HTTPStatus.OK, self.application.ai.status(enabled=True))
            return
        ...
        if method == "POST" and path == "/api/material-studio/generate":
            self._require_permission(session, "teacher.materials.use")
            self._require_permission(session, "ai.use")
            body = self._read_json_body()
            prompt = str(body.get("prompt") or "").strip()
            if not prompt:
                raise ValueError("Prompt fehlt.")
            payload = self.application.material_studio.start_generation(
                prompt=prompt,
                language=str(body.get("language") or "python"),
                profile=str(body.get("profile") or "example-code"),
                instruction_preset_key=str(body.get("instruction_preset_key") or ""),
                attempt_limit=int(body.get("attempt_limit") or 3),
                seed_code=str(body.get("seed_code") or ""),
                seed_path=str(body.get("seed_path") or ""),
            )
            self._send_json(HTTPStatus.OK, payload)
            return

        if method == "POST" and path == "/api/material-studio/infer":
            self._require_permission(session, "teacher.materials.use")
            self._require_permission(session, "ai.use")
            body = self._read_json_body()
            generation_state = dict(body.get("generation_state") or {})
            prompt = str(body.get("prompt") or "").strip()
            if not generation_state:
                raise ValueError("Material-Studio Zustand fehlt.")
            if not prompt:
                raise ValueError("Material-Studio Prompt fuer den aktuellen Schritt fehlt.")
            raw_text, model = self.application.ai.complete(
                prompt=prompt,
                system_prompt=str(body.get("system_prompt") or ""),
                response_format=dict(body.get("response_format") or {}) or None,
                generation_options=dict(body.get("generation_options") or {}) or None,
                timeout_seconds=float(body.get("timeout_seconds") or 120.0),
            )
            payload = self.application.material_studio.continue_generation(
                session,
                generation_state=generation_state,
                response_text=raw_text,
                model=model,
            )
            if payload.get("mode") != "infer":
                self.application.repository.add_audit(
                    session.username,
                    "material_studio.generate",
                    "teacher_material",
                    payload["title"],
                    {
                        "profile": payload["profile"]["key"],
        ...
        if method == "POST" and path == "/api/material-studio/run":
            self._require_permission(session, "teacher.materials.use")
            body = self._read_json_body()
            payload = self.application.material_studio.run_current(session, body)
            self.application.repository.add_audit(
                session.username,
                "material_studio.run",
                "teacher_material",
                payload["title"],
                {
                    "profile": payload["profile"]["key"],
                    "language": payload["language"],
                    "returncode": payload["run"]["returncode"],
        ...
        if method == "POST" and path in {"/api/admin/settings", "/api/server/settings"}:
            ...
            for key in ADMIN_SETTING_KEYS:
                if key in body:
                    self.application.repository.put_setting(key, body[key])
    ...
    def _serve_model_asset(self, path: str) -> None:
        segments = [unquote(segment) for segment in path.split("/") if segment]
        if len(segments) < 2:
            raise FileNotFoundError("Modelldatei nicht gefunden.")
        relative_path = "/".join(segments[1:])
        target = self._resolve_relative_file(self.application.model_root(), relative_path)
        self._serve_file(target)
    ...
    def _resolve_relative_file(root: Path, relative_path: str) -> Path:
        ...
        if any(part == ".." for part in parts):
            raise PermissionError("Ungueltiger Dateipfad.")
        ...
        try:
            ...
            raise PermissionError("Kein Zugriff auf diese Datei.") from exc
